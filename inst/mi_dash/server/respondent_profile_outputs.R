...
lapply(groups, function(x) {
  output[[glue("training_option_{x}")]] <- renderPlotly({
    stacked_vertical(data = filtered_data[[x]], columns = "training_option", radio_select_id = "Training")
  })
})
...
lapply(groups, function(x) {
  output[[glue("training_ease_rating_{x}")]] <- renderPlotly({
    stacked_vertical(data = filtered_data[[x]], columns = "training_ease_rating", radio_select_id = "Rating")
  })
})
...
lapply(groups, function(x) {
  output[[glue("system_used_option_{x}")]] <- renderPlotly({
    stacked_vertical(data = filtered_data[[x]], columns = "system_used_option", radio_select_id = "System")
  })
})
