...
lapply(groups, function(x) {
  output[[glue("training_option_{x}")]] <- renderPlotly({
    stacked_vertical(data = filtered_data[[x]], columns = "training_option")
  })
})

output$training_option_legend <- renderPlot(
  create_legend("training_option")
)

output$test_text <- renderText("blah blah")

# training_ease_rating
lapply(groups, function(x) {
  output[[glue("training_ease_rating_{x}")]] <- renderPlotly({
    stacked_vertical(data = filtered_data[[x]], columns = "training_ease_rating")
  })
})

output$training_ease_rating_legend <- renderPlot(
  create_legend("training_ease_rating")
)
...
output$system_used_legend <- renderPlot(
  create_legend("system_used")
)

# system_used_option
lapply(groups, function(x) {
  output[[glue("system_used_option_{x}")]] <- renderPlotly({
    stacked_vertical(data = filtered_data[[x]], columns = "system_used_option")
  })
})

output$system_used_option_legend <- renderPlot(
  create_legend("system_used_option")
)
